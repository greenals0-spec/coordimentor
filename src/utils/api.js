...
4. [스타일 일관성 — 절대 금지 크로스 조합]:
   아이템 이름·태그를 보고 스타일 카테고리를 판단한 뒤, 아래 금지 조합을 반드시 지켜.

   ■ 스포츠/애슬레틱 아이템 (트레이닝복·추리닝·조거팬츠·레깅스·스포츠반바지·져지 등 이름·태그에 '스포츠·트레이닝·추리닝·조거·저지·기능성' 포함):
     - 금지 신발: 구두, 로퍼, 하이힐, 옥스퍼드, 더비, 포멀부츠
     - 금지 아우터: 정장 재킷, 블레이저(수트용), 코트(포멀)
     - 금지 액세서리: 넥타이, 포멀 벨트, 가죽 벨트

   ■ 포멀/정장 아이템 (슬랙스·드레스셔츠·블레이저·수트·정장바지·vest 등):
     - 금지 신발: 슬리퍼, 샌들, 크록스
     - 금지 하의: 트레이닝바지, 추리닝, 레깅스, 조거팬츠

   ■ 캐주얼 아이템 (청바지·반바지·후드티·맨투맨·티셔츠 등):
     - 금지 액세서리: 포멀 벨트 (청바지+캐주얼 상의 조합에 벨트 제외)

   ■ 핵심 원칙: 하나의 코디 안에서 스포츠 계열 하의와 포멀 계열 신발/상의를 절대 함께 쓰지 마. 반대도 마찬가지. 각 아이템의 이름과 태그를 먼저 확인하고 스타일 카테고리를 분류한 뒤 코디를 구성해.
...
7. [의상 규칙] 벨트는 드레스셔츠+슬랙스 조합일 때만 포함. 캐주얼·스포츠 복장에는 절대 제외.
